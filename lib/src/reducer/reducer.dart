...
AppState reducer(AppState appState, dynamic action) {
  if (action is GetCryptosSuccessful) {
    ...
    return appState.copyWith(
        cryptoCoins: appState.cryptoCoins, isLoading: false);
  } else if (action is GetCryptos) {
    return appState.copyWith(
        cryptoCoins: appState.cryptoCoins, isLoading: true);
  }
  ...
  if (action is GetCryptoFromDBSuccessful) {
    ...
    cryptos.addAll(appState.cryptoCoins);
    final int index =
        cryptos.indexWhere((CryptoCoin coin) => coin.id == action.coin.id);
    cryptos[index] = CryptoCoin(
        savedCoin.id, savedCoin.name, savedCoin.symbol, savedCoin.price, true);
